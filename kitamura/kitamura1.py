...
if __name__=='__main__':
    ...
    #Rcmdr用のファイル
    f=open(f"{cwd}\\recordファイル\\record_gradient_1_p={str(percentile)}.txt","a",encoding="UTF-8")
    ...
    for a in range(6):
        ...
        for nth_time in range(number_of_simulation):
            ...
            #SNRから騒音の音圧2乗平均を求める
            nA_rms=cal_adjusted_noise_rms(sA_rms,SNR)
            ...
            f_n_list,cf,t_n_list=octavefilter(x=n,fs=sr,fraction=3,order=6,limits=[90,11000],show=False,sigbands=True)
            ...
            for one_array in n_gradient:
                n_threshold[idx]=np.percentile(one_array,percentile)  #バンドごとの閾値の設定
                idx+=1
            ...
            for one_array in mix_gradient: #バンドごとで比較
                ...
                for one_gradient in one_array: #mixの回帰直線の傾きとnの傾きの閾値を比較
                    if one_gradient>n_threshold[idx]: 
                        score+=1 #傾きが大きければ加点
    ...
    #グラフ用ファイルの作成
    g=open(f"{cwd}\\forgraph保存\\forgraph_gradient_1_p={percentile}.txt","a",encoding="UTF-8")
    for SNR in SNR_box:
        g.write(f"{str(SNR)} ")
    g.write("\n")
