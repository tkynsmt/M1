#あるSN比、パーセンタイル値でのホワイトノイズのバンド毎の閾値を書き出す
...
for a in range(6):
    ...
    #SAのrms、SN比からnAのrmsを求める
    nA_rms=cal_adjusted_noise_rms(sA_rms,SNR)

    #1つのSN比につきnのパーセンタイル値を50回求める
    threshold_50x21=np.zeros(50)
    for i in range(50):

        #nの作成
        n1=generate_pink_noise(sr*10)    #n1の作成（10秒）
        ...
        for one_array in n_gradient:
            n_threshold[idx]=np.percentile(one_array,percentile)
        
        threshold_50x21[i]=n_threshold
    

    #バンド毎でパーセンタイル値の平均値を算出
    threshold_mean=np.zeros(len(n_threshold))   #あるSN比でのバンド毎の閾値の平均値
    threshold_21x50=np.transpose(threshold_50x21)
    idx=0
